...
export class ProductTypeComponent implements OnInit {
    ...
    confirmdelete() {
        if (this.deleteId !== undefined && this.deleteId != '') {
            ...
            this.productTypeService.delete(this.deleteId).subscribe((response: any) => {
                ...
                this.spinner.hide();
                this.sendAnyMessageErro(this.toastr, error, 'Exclusão do tipo de produto')
            });
            this.deleteId == '';
            this.modalVisible = false;
            var filter = new ProductTypeFilter('', 'todos', this.firstPage, this.formFilter.value);
    ...
    prepareDelete(id: string, name: string) {
        this.deleteId = id;
        this.bodyDetailTodelete = `Deseja excluir o tipo de produto (${name}) ?`;
